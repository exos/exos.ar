...
export default function TPage() {
    const [contactStatus, setContactStatus] = useState<ContactStatus>(ContactStatus.notload);
    ...
    const fetchContactInfo = useCallback(async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch contact info:', response.statusText, response.status);

                if (response.status === 400) {
                    const data = await response.json();
                    console.log('data', data);
                    if (typeof data.error === 'string' && data.error === 'Captcha is required') {
        ...
        <main className="min-h-screen bg-black text-green-500 items-center mx-auto max-w-5x1 px-6 py-12">
            ...
            <div className="mx-auto max-w-5xl px-6 py-12">
                ...
                <section className="mt-10 rounded-xl border border-green-500/20 bg-green-500/5 p-6">
                    ...
                    {( contactStatus === ContactStatus.loading || contactStatus === ContactStatus.notload) && (
                    ...
                    {( contactStatus === ContactStatus.failed) && (
                        <div className="mx-auto max-w-5xl px-6 py-12 bg-red-100 rounded-md">
                            <span className="block text-red-700 mb-2">Sorry, we have a problem here</span>
                            <a
